...
def user_message(db, conversation):
    return Message.objects.create(conversation=conversation, role='user', content='How do I reset my password?')


@pytest.fixture
def assistant_message(db, conversation):
    return Message.objects.create(conversation=conversation, role='assistant', content='Click forgot password.')
...
def test_summary_total_messages_counts_user_messages(auth_client, chatbot, user_message, assistant_message):
    resp = auth_client.get(f'/api/analytics/{chatbot.id}/summary/')
    assert resp.status_code == 200
    assert resp.data['total_messages'] == 1  # only user messages counted
...
def test_frequent_questions_sorted_by_frequency(auth_client, chatbot, conversation):
    # "common question" asked 3 times, "rare question" asked once
    for _ in range(3):
